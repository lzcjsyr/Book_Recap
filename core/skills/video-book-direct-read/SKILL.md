...
- `references/writing-standard.md`：视频号读书解说稿的具体文稿要求。
- `references/revision-workflow.md`：逐轮初稿、忠实性稿、结构稿、口语稿、终稿和差异审计的可复盘落盘流程。
- `references/output-contract.md`：最终 raw JSON 字段、数量、格式和质量要求。
...
4. 输出前读 `references/output-contract.md`，确保最终 JSON 可被 `json.loads` 解析，并符合当前项目 Step 1 raw JSON 需求。
